...
//    std::string output_path = "E:\\pig1-150\\xyz150\\xyz150\\150\\chest_width\\points.pcd";
//
//    // 创建点云指针
...
//        return -1;
//    }
//    pcl::PointXYZ centroid = computeCentroid(front_part);
//
...
//
//    for (const auto& point : front_part->points) {
...
//
//    // 切出前后 5cm 的切片
//    pcl::PointCloud<pcl::PointXYZ>::Ptr front_slice = slicePointCloud(front_part, front_slice_min_x, front_slice_max_x);
//    pcl::PointCloud<pcl::PointXYZ>::Ptr back_slice = slicePointCloud(front_part, back_slice_min_x, back_slice_max_x);
//
//    pcl::PointXYZ min = findMin(front_slice);
//    pcl::PointXYZ max = findMax(back_slice);
//    pcl::PointXYZ P1 = findP1(front_part);
//    pcl::PointXYZ P2 = findP2(front_part);
//    float A, B, C, D;
//    fitPlane(max, min, A, B, C, D);
//
//    double totalDistance = findMaxDistanceSum(front_part, A, B, C, D);
//    float resolution = 0.150; 
//    //std::cout<< "totalDistance = " << totalDistance << endl;
//    //savePlaneAsXYZ(A, B, C, D, output_path, front_part, resolution);
//    //pcl::io::savePCDFileASCII("E:\\Pig_Cattle\\Pig\\label\\after_label_two\\80forseg\\cut\\chest_width\\56_cen.pcd", *front_part);
//    std::cout << "Hip_height =  " << totalDistance << std::endl;
//    std::ofstream output_file("E:\\pig1-150\\xyz150\\xyz150\\150\\Hip_height\\distance.txt");  // 打开文件（会自动创建或覆盖）
//
//    if (output_file.is_open()) {
//        // 将曲线长度写入文件
//        output_file << "Hip_height: " << totalDistance << std::endl;
//        output_file.close();  // 关闭文件
//        std::cout << "Hip_height saved to curve_length.txt" << std::endl;
//    }
//    else {
